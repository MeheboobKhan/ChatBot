...
#lines1 = open('movie_lines.txt',encoding='utf-8',errors='ignore').read()
#print(lines[0])
#l=lines[0].split(' +++$+++ ')
#print(l)
#len(l)
...
def clean_text(text):
    ...
    text = re.sub(r"won't", "wil not", text)
    text = re.sub(r"don't", "do not", text)
    text = re.sub(r"can't", "cannot", text)
    text = re.sub(r"didn't", " did not", text)
    text = re.sub(r"wouldn't", " would not", text)
    text = re.sub(r"where's", "where is", text)
    text = re.sub(r"\'ll", " wil", text)
...
for qstn in questions:
    clean_questions.append(clean_text(qstn))

clean_answers = []
for answr in answers:
    clean_answers.append(clean_text(answr))

#creating a dictionary that maps each word to its number of occurences
word2count = {}
for qstn in clean_questions:
...
for answr in clean_answers:
    for word in answr.split():
        if word not in word2count:
            word2count[word]=1
        else:
            word2count[word]+=1
...
# creating inverse dictionary of the answerswords2int dictionary, required for seq 2 seq model
answersint2word = {w_i:w for w,w_i in answersword2int.items()}

#adding the EOS token to the end of every answer, required for seq 2 seq model
...
# helps in optimizing the training

questions_to_int = []
for qstn in clean_questions:
    ints=[]
    for l in qstn.split():
        if l not in questionsword2int:
            ints.append(questionsword2int['<OUT>'])
        else:
            ints.append(questionsword2int[l])
...
for ans in clean_answers:
    ints=[]
    for l in ans.split():
        if l not in answersword2int:
            ints.append(answersword2int['<OUT>'])
        else:
            ints.append(answersword2int[l])
...
#len_clean_questions[i] = len(clean_questions[i])
#len_clean_questions = {}
#newdict = {}
#len_clean_questions = sorted(len_clean_questions.items(), key = lambda t:t[1])
#newdict = {w_i:w for w,w_i in len_clean_questions.items()}

#for key in len_clean_questions:
#sorted_clean_questions.append(clean_questions[key])
...
#Creating the decoder training set
